$(document).ready(function() {
    ...
    $('.link-more').click(function(e) {
        e.preventDefault();
        $(this).parent().find('.link-list_hidden').slideToggle();
    });
    ...
    $(".switch__button").on("click", function() {
        ...
        if ($button.parent().hasClass('product__amount')) {
            ...
            var topParent = $button.parents('.product__item'),
                daysProduct = topParent.find('.product__days'),
                daysProductHtml = daysProduct.html(),
                totalProduct = newVal * 1 / 100,
                totalProductInner = newVal + (totalProduct * parseFloat(daysProductHtml)),
                resTag = topParent.find('.product__total');
            resTag.html(totalProductInner + ' ');
            ...
            var topParent = $button.parents('.product__item');
            var sumProduct = topParent.find('.product__num').html();
            var totalProduct = +sumProduct * 1 / 100;
            var totalProductInner = +sumProduct + (totalProduct * parseFloat(newVal));
            resTag = topParent.find('.product__total');
            resTag.html(totalProductInner + ' ');
